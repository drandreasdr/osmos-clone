...
pub fn handle_ejection(scene: &mut scene::Scene, target_coordinates: Vector2<f64>) {
    ...
    let aim_direction = (target_coordinates - scene.cell_collection.get_player().position).normalize();

    let new_enemy_start_position =
        scene.cell_collection.get_player().position + aim_direction * (scene.cell_collection.get_player().radius + RADIUS_EJECTED_PARTICLE);

    let velocity_ejected_particle =
        RELATIVE_VELOCITY_MAGNITUDE_EJECTED_PARTICLE * aim_direction + scene.cell_collection.get_player().velocity;

    let area_player = PI * scene.cell_collection.get_player().radius * scene.cell_collection.get_player().radius;
    let area_ejected_particle = PI * RADIUS_EJECTED_PARTICLE * RADIUS_EJECTED_PARTICLE;

    let new_velocity_player = area_player / (area_player - area_ejected_particle) * scene.cell_collection.get_player().velocity
    ...
    scene.cell_collection.get_player().velocity = new_velocity_player;
}
